        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h4 class="card-title mb-0 fw-bold text-dark"><i class="icon-notebook me-2 text-primary"></i>Add New Chapter</h4>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="course_id" id="course_id" class="form-select form-control">
                                ...
                                @foreach($courses as $crs)
                                    <option value="{{ $crs->id }}" {{ old('course_id') == $crs->id ? 'selected' : '' }}>
                                        {{ $crs->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="chapter_number" id="chapter_number" class="form-control" 
                                   placeholder="e.g. 1, 2, or Chapter 1" value="{{ old('chapter_number') }}">
                            <small class="text-muted d-block mt-1">e.g. 1, 2, 3</small>
                        </div>
                        <div class="col-md-8">
                            <label for="name" class="form-label fw-semibold">Chapter Name / Title <span class="text-danger">*</span></label>
                            <input type="text" name="name" id="name" class="form-control" 
                                   placeholder="e.g. Cell: The Unit of Life" value="{{ old('name') }}" required>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label fw-semibold">Description <small class="text-muted">(Optional)</small></label>
                        <textarea name="description" id="description" rows="3" class="form-control" placeholder="Brief details about topics covered in this chapter...">{{ old('description') }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label for="sort_order" class="form-label fw-semibold">Sort Order</label>
                        <input type="number" name="sort_order" id="sort_order" class="form-control" value="{{ old('sort_order', 0) }}" min="0" style="max-width: 150px;">
                    </div>

                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('admin.chapters.index') }}" class="btn btn-secondary">Cancel</a>
                        <button type="submit" class="btn btn-primary fw-semibold"><i class="icon-check me-1"></i> Save Chapter</button>
                    </div>
                </form>
